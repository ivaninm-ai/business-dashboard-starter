...
- 一旦修改了 CSV，`test/expected/` 的答案和 `src/briefs/examples.js` 的示例分析就不再适用：请同时更新它们（见 `docs/STUDENT_SOP.md`「换成你自己的数据」）。Once you change the records, the answer keys and the prepared briefs no longer apply — update them too.
